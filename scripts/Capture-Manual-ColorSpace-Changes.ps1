...
function Capture-State {
    ...
    & $ControlMyMonitor /scomma $csvPath $Monitor
    & $ControlMyMonitor /stext $txtPath $Monitor

    if (-not (Test-Path $csvPath)) {
        throw "CSV capture was not created: $csvPath"
    }
...
function Compare-Snapshots {
        ...
        foreach ($field in $fields) {
            ...
            if ($beforeValue -ne $afterValue) {
                $diffs.Add([pscustomobject]@{
                    ...
                    To = $CurrentLabel
                    VCPCode = $code
                    VCPName = if ($after.Name) { $after.Name } else { $before.Name }
                    Field = $field
                    Before = $beforeValue
                    After = $afterValue
                })
    ...
    if ($diffs.Count -eq 0) {
        ...
        @() | Export-Csv -Path $diffCsv -NoTypeInformation -Encoding UTF8
    }
    else {
...
& $ControlMyMonitor /smonitors $monitorsFile
